...
@NamedQueries({
        @NamedQuery(
                name = "BasicIngredient.findByIngredientsSum",
                query = "SELECT i FROM BasicIngredient AS i " +
                        "GROUP BY i.id " +
                        "HAVING SUM(i.price) < :sum_price"
        ),
        @NamedQuery(
                name = "BasicIngredient.findAllIngredientsAndShampooBrands",
                query = "SELECT i.name, s.brand " +
                        "FROM BasicIngredient AS i " +
                        "LEFT JOIN i.shampoos AS s"
        ),
        @NamedQuery(
                name = "BasicIngredient.deleteByName",
                query = "DELETE FROM BasicIngredient AS i " +
                        "WHERE i.name = :name"
        ),
        @NamedQuery(
                name = "BasicIngredient.increasePrice",
                query = "UPDATE BasicIngredient AS i " +
